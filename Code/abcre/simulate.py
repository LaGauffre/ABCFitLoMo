...
def simulate_claim_data(rg, T, freq, sev, theta):
    ...
    if freqs.sum() > 1e7:
        print(f"PROBLEM! Theta={theta} => Asked to simulate a gigantic number of claims.")
        print("Specifically, freqs = ", freqs)
        ...
        while freqs.sum() > 1e7:
            ...
            freqs[i] = 0
            nRemoved += 1
            #print(f"[{nRemoved}] Reducing {s} to {np.sum(freqs)} by removing freqs[{i}] = {rem}") 
        print(f"The sum is now {np.sum(freqs)} after {nRemoved}/{T} edits")
